...
def wrap_batch_pcs_future(batch, num_frames=5):
    ...
    Returns transformed point clouds and pose flows for each consecutive pair.
    """
    batch_sizes = len(batch["pose0"])

    # Store transformed point clouds for each frame
    # All frames will be transformed to pc1's coordinate system for consistency
    pcs_dict = {}

    for batch_id in range(batch_sizes):
        # Reference pose is pc1's pose (the target of pc0->pc1 flow)
        ...
        with torch.no_grad():
            ...
            pc0_transformed = batch["pc0"][batch_id] @ pose_0to1[:3, :3].T + pose_0to1[:3, 3]

            if batch_id == 0:
                pcs_dict['pc0s'] = [pc0_transformed]
                pcs_dict['pc1s'] = [batch["pc1"][batch_id]]  # pc1 is in its own coordinate
                pcs_dict['pose_flow_0'] = [pc0_transformed - batch["pc0"][batch_id]]
            else:
                pcs_dict['pc0s'].append(pc0_transformed)
                pcs_dict['pc1s'].append(batch["pc1"][batch_id])
                pcs_dict['pose_flow_0'].append(pc0_transformed - batch["pc0"][batch_id])

            # Transform future frames (pc2, pc3, pc4, ...) to their respective coordinate systems
            for i in range(2, num_frames):
                pc_key = f'pc{i}'
                pose_key = f'pose{i}'

                if pc_key in batch:
                    # For frame i, we compute pose from frame i-1 to frame i
                    # This gives us the transformation to apply to points in frame i-1 coords
                    prev_pose_key = f'pose{i-1}'
                    pose_prev_to_i = cal_pose0to1(batch[prev_pose_key][batch_id], batch[pose_key][batch_id])

                    pc_i = batch[pc_key][batch_id]

                    if batch_id == 0:
                        pcs_dict[f'pc{i}s'] = [pc_i]
                        # Pose flow for frame i-1 -> i
                        # We need to transform pc{i-1} to pc{i}'s coordinate and compute the displacement
                        pc_prev = batch[f'pc{i-1}'][batch_id]
                        pc_prev_transformed = pc_prev @ pose_prev_to_i[:3, :3].T + pose_prev_to_i[:3, 3]
                        pcs_dict[f'pose_flow_{i-1}'] = [pc_prev_transformed - pc_prev]
                    else:
                        pcs_dict[f'pc{i}s'].append(pc_i)
                        pc_prev = batch[f'pc{i-1}'][batch_id]
                        pc_prev_transformed = pc_prev @ pose_prev_to_i[:3, :3].T + pose_prev_to_i[:3, 3]
                        pcs_dict[f'pose_flow_{i-1}'].append(pc_prev_transformed - pc_prev)
